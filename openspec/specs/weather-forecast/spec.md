...
The system SHALL serve weather data for exactly three hardcoded locations: Castlemaine, Melbourne, and Sorrento (all Victoria, AU). These locations SHALL be defined in `backend/config.yaml`, each with `city`, `country`, `bom_url` fields.
